 ...
 *
 */
class assignment_simpleupload extends assignment_base {

    function assignment_simpleupload($cmid='staticonly', $assignment=NULL, $cm=NULL, $course=NULL) {
        parent::assignment_base($cmid, $assignment, $cm, $course);
        $this->type = 'simpleupload';
    }
    ...
    function view() {
        ...
        if (is_enrolled($this->context, $USER)) {
            ...
            if ($filecount and $submission) {
                echo $OUTPUT->box($this->print_user_files($USER->id, true), 'generalbox boxaligncenter simpleupload', 'userfiles');
            } else {
    ...
    function print_student_answer($userid, $return=false){
        ...
        if ($this->notes_allowed() and !empty($submission->data1)) {
            $link = new moodle_url("/mod/assignment/type/simpleupload/notes.php", array('id'=>$this->cm->id, 'userid'=>$userid));
    ...
    function print_user_files($userid=0, $return=false) {
        ...
        $candelete = $this->can_delete_files();
    ...
    function upload_notes() {
        ...
        $mform = new mod_assignment_simpleupload_notes_form();
...
class mod_assignment_simpleupload_notes_form extends moodleform {
...
class mod_assignment_sipmleupload_response_form extends moodleform {
    function definition() {
        $mform = $this->_form;
